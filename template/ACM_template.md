...
// 模板函数：生成随机数值数组
template <typename T>
std::vector<T> generate_random_with_time(size_t length, T min, T max) {
    // 1. 获取当前时间作为种子 (纳秒级)
    ...
    if constexpr (std::is_integral_v<T>) {
        ...
        for (size_t i = 0; i < length; ++i) data.push_back(dist(gen));
    } else {
...
int main() {
    ...
    // 每次运行结果都会随时间改变
    auto results = generate_random_with_time<int>(len, 1000, 9999);

    std::cout << "基于时间种子的随机数组: " << std::endl;
    for (int val : results) {
        std::cout << "[" << val << "] ";
    }
...
struct DoublePointer{
    ...
    void solve(){
        ...
        int l=1;
        for(int r=1;r<=n;r++){//for循环拓展右端点，左端点收缩，这样好写一些
            if(a[r]>x){
                mp.clear();
                l=r+1;
            }
...
struct Fenwick_Tree {
    ...
    void add(int x, int d) {
        // 从 x 开始，逐层向上更新树状数组
        for (; x <= len; x += x & -x) {  // x & -x 计算出 x 的最低位的 1 表示的值
            C[x] += d;  // 将 d 加到 C[x] 上
        }
    }
    ...
    void down(int x) {
        ...
        if (u.sam) {
            ...
            if (u.v > 0) {
                ...
                if (u.s[1])rs.mx = rs.v, ls.rs = rs.rs = 0;
            }
        }
    }
...
**1.** update(int u)：信息上传与合并（Push-up）
* 功能：类似于线段树，将左右子树的信息合并到父节点。它是维护最大子段和的核心逻辑。

**2.** split 与 merge：平衡树结构维护功能：
* split 按索引大小将一棵 Treap 拆成两棵；
* merge 将两棵有序的 Treap 合并。实现细节：
    * 由于 Treap 节点本身就是数组索引（$1 \dots N$），索引天然有序。通过 update(u) 在每次旋转或结构变动后重新计算最大子段和信息。


时间复杂度： $O(\log n)$，但常数比线段树大

```cpp
/**
 * FHQ Treap 模板
 * 功能：维护一个有序集合，支持插入、删除、查询前 k 大元素的和。
 * 优势：代码短，易于理解，无需旋转。
 */
struct FHQ_Treap {
    struct Node {
        int l, r;       // 左右子节点
        int val;        // 节点权值 
        unsigned int priority; // 随机优先级
        int sz;         // 子树大小
        long long sum;  // 子树权值和
    };

    vector<Node> tr;
    int root, idx;      // 如果需要开多棵树，则使用 root[N] 即可
    mt19937 rng;

    FHQ_Treap(int n) : tr(n + 10), root(0), idx(0), rng(1337) {}

    void init() {
        root = 0;
        idx = 0;
        // 注意：这里不需要循环初始化所有节点，随用随开即可
    }
    ...
    void pushup(int u) {
        ...
        // 步骤 A：合并左子树信息
        if (l) {
            int nsm=tr[l].sum+tr[u].sum;
            int nsiz=tr[l].sz+tr[u].sz;
            tr[u].sz=nsiz;
            tr[u].sum=nsm;
        }
        ...
        // 步骤 B：合并右子树信息（逻辑同上）
        if (r) {
            int nsm=tr[r].sum+tr[u].sum;
            int nsiz=tr[r].sz+tr[u].sz;
            tr[u].sz=nsiz;
            tr[u].sum=nsm;
        }
    }
    ...
    /**
     * 按权值分裂
     * l 子树所有节点的 val <= v，r 子树所有节点的 val > v
     */
    void split_by_val(int u, int v, int &l, int &r) {
        if (!u) { l = r = 0; return; }
        if (tr[u].val <= v) {
            l = u;
            split_by_val(tr[u].r, v, tr[l].r, r);
        } else {
            r = u;
            split_by_val(tr[u].l, v, l, tr[r].l);
        }
        pushup(u);
    }
     ...
     * l 子树包含前 k 个节点
     */
    void split_by_sz(int u, int k, int &l, int &r) {
        if (!u) { l = r = 0; return; }
        if (tr[tr[u].l].sz + 1 <= k) {
            l = u;
            split_by_sz(tr[u].r, k - tr[tr[u].l].sz - 1, tr[l].r, r);
        } else {
            r = u;
            split_by_sz(tr[u].l, k, l, tr[r].l);
        }
        pushup(u);
    }
    ...
    void insert(int v) {
        int l, r;
        split_by_val(root, v, l, r);
        root = merge(merge(l, new_node(v)), r);
    }

    /**
     * 查询最大的 k 个元素的和
     * 逻辑：分裂出大小为 (total_size - k) 的左子树，剩余的右子树即为最大的 k 个。
     */
    long long query_top_k(int k) {
        if (k <= 0) return 0;
        if (k >= tr[root].sz) return tr[root].sum;
        int l, r;
        split_by_sz(root, tr[root].sz - k, l, r);
        long long res = tr[r].sum;
        root = merge(l, r); // 别忘了合并回去
        return res;
    }
};
```
...
1. 核心思想：懒标记 (Lazy Tag)类似于线段树的区间修改，我们在 Treap 的节点上维护一个 rev 标记。含义：如果当前节点的 rev 为 1，表示该节点的左右子树需要互换，且其子孙节点也需要递归翻转。操作：当我们访问一个节点时，先执行 pushdown。如果标记存在，交换当前节点的左右儿子，并将标记下传给子节点。
2. 区间翻转的步骤假设我们要翻转区间 $[L, R]$：
* Split：按子树大小（Size）将整棵树分裂成三部分：
    * $T_1$：前 $L-1$ 个元素。
    * $T_2$：区间 $[L, R]$ 对应的子树。
    * $T_3$：第 $R+1$ 个元素之后的部分。
* Tagging：给 $T_2$ 的根节点打上一个 rev 标记（tr[T2].rev ^= 1）。
* Merge：按顺序将 $T_1, T_2, T_3$ 重新合并。

```cpp

/**
 * FHQ Treap 全能模板
 * 支持：插入、删除、排名、第K大、前驱、后继
 * 支持：区间翻转 (Lazy Tag)
 */
struct FHQ_Treap {
    struct Node {
        int l, r;           // 左右儿子
        int val;            // 节点值
        unsigned int key;   // 随机优先级
        int sz;             // 子树大小
        bool rev;           // 翻转标记 (根据题目需求可添加其他 lazy tag)
        long long sum;      // 示例：区间和
    };
    ...
    void pushup(int u) {
        ...
        tr[u].sum = tr[tr[u].l].sum + tr[tr[u].r].sum + tr[u].val; // 维护区间和示例
    }
    ...
    void split_by_val(int u, int v, int &l, int &r) {
        ...
        if (tr[u].val <= v) {
            l = u;
            split_by_val(tr[u].r, v, tr[u].r, r);
        } else {
            r = u;
            split_by_val(tr[u].l, v, l, tr[u].l);
        }
        pushup(u);
    }

    // 按排名/大小分裂：l 子树包含前 k 个元素
    void split_by_sz(int u, int k, int &l, int &r) {
        if (!u) { l = r = 0; return; }
        pushdown(u);
        int cur_sz = tr[tr[u].l].sz + 1;
        if (cur_sz <= k) {
            l = u;
            split_by_sz(tr[u].r, k - cur_sz, tr[u].r, r);
        } else {
            r = u;
            split_by_sz(tr[u].l, k, l, tr[u].l);
        }
        pushup(u);
    }
    ...
    void remove(int v) {
        int l, m, r;
        split_by_val(root, v, l, r);
        split_by_val(l, v - 1, l, m);
        m = merge(tr[m].l, tr[m].r); // 合并 m 的左右儿子即删除了根节点
    ...
    int get_kth(int k) {
        int l, m, r;
        split_by_sz(root, k, l, r);
        split_by_sz(l, k - 1, l, m);
        int res = tr[m].val;
        root = merge(merge(l, m), r);
        return res;
    }
    ...
    int get_prev(int v) {
        ...
        split_by_val(root, v - 1, l, r);
        int u = l;
        while (tr[u].r) { pushdown(u); u = tr[u].r; }
        int res = tr[u].val;
        root = merge(l, r);
        return res;
    }
    ...
    void reverse(int L, int R) {
        int l, m, r;
        split_by_sz(root, L - 1, l, m);
        split_by_sz(m, R - L + 1, m, r);
        if (m) tr[m].rev ^= 1;
...
$$
h[i][j]=h[i-1][j]+h[i][j-1]-h[i-1][j-1]+s[i][j]
$$
...
struct HASH_2{
    int maxn=1e6;
    int RP=107,CP=61;//RP为行方向的哈希基数，CP为列方向的哈希基数，这两个数比 131 和 1331 更好，冲突少
    int rpow[N],lpow[N];// rpow[i]表示RP的i次方，lpow[i]表示CP的i次方
    int inv_rpow[N],inv_cpow[N];// inv_rpow[i]表示RP的i次方的逆元，inv_cpow[i]表示CP的i次方的逆元
    vector<vector<int>>h;// h[i][j]表示从(1,1)到(i,j)的子矩阵的哈希值
    ...
    void init(){
        ...
        for(int i=1;i<=maxn;i++){
            rpow[i]=mul(rpow[i-1],RP);
            lpow[i]=mul(lpow[i-1],CP);
        }
        inv_rpow[0] = 1;
        inv_cpow[0] = 1;
        int base_invr = qpow(RP, mod-2, mod);
        int base_invc = qpow(CP, mod-2, mod);
        for(int i=1; i<=maxn; i++) {
            inv_rpow[i] = mul(inv_rpow[i-1], base_invr);
            inv_cpow[i] = mul(inv_cpow[i-1], base_invc);
        }
    }

    void work(string (&s)[N]){
        for(int i=1;i<=n;i++){
            for(int j=1;j<=m;j++){
                int cur=(s[i][j]-'a'+1);
                int hash=mul(cur,mul(rpow[i],lpow[j]));
                h[i][j]=add(sub(add(h[i-1][j],h[i][j-1]),h[i-1][j-1]),hash);
            }
        }
    }
    ...
    int get(int x1,int y1,int x2,int y2){
        int ret=add(sub(sub(h[x2][y2],h[x2][y1-1]),h[x1-1][y2]),h[x1-1][y1-1]);
        ret=mul(ret,mul(inv_rpow[x1-1],inv_cpow[y1-1]));
        return ret;
    }
}h,th;
...
\(\text{endpos}(s)\)：子串 \(s\) 所有出现的位置（尾字母下标）集合。SAM 中的每个状态都一一对应一个 \(\text{endpos}\) 等价类。
\(\text{endpos}\) 的性质：

(1) 令 \(s_1, s_2\) 为 \(S\) 的两个子串，不妨设 \(|s_1| \leq |s_2|\)（我们用 \(|s|\) 表示 \(s\) 的长度，此处等价于 \(s_1\) 不长于 \(s_2\)）。则 \(s_1\) 是 \(s_2\) 的后缀当且仅当 \(\text{endpos}(s_1) \supseteq \text{endpos}(s_2)\)，\(s_1\) 不是 \(s_2\) 的后缀当且仅当 \(\text{endpos}(s_1) \cap \text{endpos}(s_2) = \emptyset\)。

(2) 两个不同子串的 \(\text{endpos}\)，要么有包含关系，要么没有交集。

(3) 两个子串的 \(\text{endpos}\) 相同，那么短串为长串的后缀。

(4) 对于一个状态 \(st\)，以及任意的 \(\text{longest}(st)\) 的后缀 \(s\)，如果 \(s\) 的长度满足：\(|\text{shortest}(st)| \leq |s| \leq |\text{longest}(st)|\)，那么 \(s \in \text{substrings}(st)\)。
三、SAM的构造过程
分类讨论，具体看板书。
证明较为复杂，略。
...
在下文中，把结点 u 在欧拉序列中第一次出现的位置编号记作 pos(u)（也称作节点 u 的欧拉序），把欧拉序列本身记作 E[1..2n-1]。
...
总复杂度为 循环次数 $\times$ 单次寻找增广路的开销：$$O(F \cdot \text{SPFA}) = O(F \cdot V \cdot E)$$实际运行表现与注意事项虽然理论上限看起来很高，但在算法竞赛和实际应用中，这个算法的表现通常比理论值要好：
1. **流量 $F$ 的影响**：如果图的容量很大，但实际流量较小（例如二分图匹配问题，$F \le N$），该算法跑得飞快。
2. **SPFA 的效率**：在处理费用流时，SPFA 是处理负权边（反向边费用为负）最简单的方法。只要图中没有负权环，SPFA 表现通常很稳定。
3. **数据规模建议**：该模板通常适用于 $V \approx 1000$，$E \approx 10000$ 左右的规模。在你代码中定义的 N = 5010 和 M = 100010，如果最大流 $F$ 很大且图结构复杂，可能会出现 TLE (超时)。
...
struct EK_SuanFa {
    ...
    void add(int a, int b, int c, int d) {
        ...
        // 反向边：容量为 0，费用为正向边的相反数 -d
        e[idx] = a, f[idx] = 0, w[idx] = -d, ne[idx] = h[b], h[b] = idx++;
    }
    void init(){
        idx = 0;
        memset(h, -1, sizeof h);
        memset(f, 0, sizeof f); // 养成好习惯，虽然全局变量自动为0
    }
...
特性|匈牙利算法 (Hungarian/KM BFS)|EK 费用流 (EK + SPFA)
---|---|---
本质类型|专门针对二分图最优匹配|通用的最小费用最大流
图的类型|必须是二分图|任意有向/无向图
时间复杂度|$O(n^3)$ 或 $O(n^2 m)$|$O(F \cdot E \cdot V)$ (通常较慢)
建模难度|极低（直接填入代价矩阵）|较高（需建立源点、汇点、容量和费用边）
容量限制|隐含为 1（每人配一题）|可自定义容量（每人可做 $k$ 题）
代码量|较精简|较多（结构体及各种数组）

```cpp
/**
 * 匈牙利算法模板 (最小权完美匹配) 
 * 复杂度: O(n^2 * m)，n 为左侧集合大小，m 为右侧集合大小
 * @param cost 代价矩阵，cost[i][j] 表示左侧第 i 个点匹配右侧第 j 个点的费用
 * 返回最小权完美匹配的代价
 */
template<typename T>
T hungarian(const vector<vector<T>>& cost) {
    const T INF = numeric_limits<T>::max();
    int n = cost.size();    // 左侧点数 (任务/人)
    int m = cost[0].size(); // 右侧点数 (位置/物品)
    
    // u[i] 和 v[j] 是行和列的“顶标”（Potentials/势能）
    // 满足性质：u[i] + v[j] <= cost[i][j]
    // 当等号成立时，该边进入“相等子图”，可以用于匹配
    vector<T> u(n + 1), v(m + 1), dist(m + 1);
    
    // p[j] 存储右侧第 j 列匹配的左侧行号，p[j] = i
    // way[j] 记录前驱列，用于在找到增广路后回溯修改匹配关系
    // used[j] 标记列在当前步骤中是否已被访问
    vector<int> p(m + 1), way(m + 1), used(m + 1);

    // 遍历每一行，为每一行 i 寻找一个匹配
    for (int i = 1; i <= n; ++i) {
        p[0] = i;      // 暂存当前行
        int j0 = 0;    // 当前考察的列索引（从虚拟的 0 列开始）
        fill(dist.begin(), dist.end(), INF); // 初始化松弛变量
        ...
        do {
            ...
            j0 = j1; // 移动到下一列
        } while (p[j0] != 0); // 直到找到一个未匹配的列

        // 通过 way 数组回溯，更新增广路径上的匹配关系
        do {
            int j1 = way[j0];
            p[j0] = p[j1];
            j0 = j1;
...
void solve(){
    ...
    vector<vector<int>>c(n,vector<int>(2*n));
    for(int i=0;i<n;i++){
        for(int j=0;j<2*n;j++){
            c[i][j]=abs(a[i]-j);
        }
...
struct PRIME {
	vector<int>prime;
	int s[N];// s[i] 表示 i 的一个质因数
	// 初始化函数，使用线性筛法计算最小质因数
	void get_prime(int n = 1e6 + 10) {
		for (int i = 2; i < n; i++) {
			// 此时如果 s[i] 为 0，说明 i 是质数
			if (!s[i]) {
				// 记录质数 p 和最小质因数 s
				prime.push_back(i);
				s[i] = i;
...
**步长优化**：代码先处理了质数 $2$，随后的循环 i += 2 跳过了所有偶数，常数时间直接缩减一半。

**$O(\sqrt{n})$ 复杂度**：这是处理单个数分解的理论最优解（除非使用更复杂的 Pollard's Rho 算法）。对于 $10^9$ 的数据，循环最多跑 $31622$ 次，飞快。

**注意（多组询问）**：如果需要对很多个较小的数（比如 $10^6$ 以内）进行分解，建议先用埃氏筛或欧拉筛预处理出最小质因子，这样分解速度可以达到 $O(\log n)$。

```cpp
/**
 * 质因数分解模板
 * 返回值: vector<pair<long long, int>> 
 * 其中 pair.first 是质因子，pair.second 是该质因子的幂次
 */
vector<pair<long long, int>> prime_factorization(long long n) {
    vector<pair<long long, int>> factors;
    
    // 1. 处理因子 2 (单独处理可以优化后续循环步长)
    if (n % 2 == 0) {
        int cnt = 0;
        while (n % 2 == 0) {
            n /= 2;
    ...
    // 2. 处理奇数因子
    // 只需要枚举到 sqrt(n)，因为如果 n 有大于 sqrt(n) 的因子，最多只有一个
    ...
    // 3. 最后如果 n > 1，说明剩下的 n 是一个大于原 sqrt(n) 的质因子
    if (n > 1) {
...
void demo() {
    long long p = 100;
    auto result = prime_factorization(p);
...
1. **线性预处理 ($O(N)$)**:
 传统的埃氏筛会重复标记合数（比如 $12$ 会被 $2$ 筛一次，又被 $3$ 筛一次）。而欧拉筛通过 p > min_prime[i] 的判断，确保了 $12$ 只会被它的最小质因子 $2$ 筛掉。
2. **极速分解 ($O(\log n)$)**:
 普通的分解需要从 $2$ 试除到 $\sqrt{n}$。而有了 min_prime 数组，你就像有了“作弊代码”：
    - 看到 $120$，查表知道最小质因子是 $2$。
    - $120 \div 2 \div 2 \div 2 = 15$。
    - 查表知道 $15$ 的最小质因子是 $3$。
    - $15 \div 3 = 5$。
    - 查表知道 $5$ 的最小质因子是 $5$。
    - 结束。每次操作都至少让 $n$ 缩小，步数极少。
### ⚠️ 使用建议
空间换时间： 这个算法需要一个和 $N$ 一样大的 int 数组。如果 $N=10^7$，大约占用 $40\text{MB}$ 内存。在内存限制严格（如 $64\text{MB}$）的题目中要小心。适用场景： 题目中有成千上万个数字需要分解，且数字大小都在 $10^7$ 以内。LL 问题： 如果输入的 $n$ 大于 MAXN（比如 $10^{12}$），这个方法就失效了，必须回退到 $O(\sqrt{n})$ 的试除法。
```cpp
#include <vector>
#include <algorithm>

/**
 * FastFactorizer - 基于线性筛的快速分解模板
 * 预处理复杂度: O(N)
 * 分解复杂度: O(log n)
 * 适用范围: n <= 10^7 (受限于内存)
 */
...
private:
    int min_prime[MAXN + 1]; // 存储最小质因子 (Smallest Prime Factor)
    std::vector<int> primes; // 存储范围内的所有质数

public:
    FastFactorizer() {
        std::fill(min_prime, min_prime + MAXN + 1, 0);
    ...
    bool is_prime(int n) const {
        if (n < 2 || n > MAXN) return false;
        return min_prime[n] == n;
    }

    // 2. 核心功能：快速质因数分解 (返回 {质因子, 幂次})
    std::vector<std::pair<int, int>> get_factors(int n) const {
        ...
        // 如果 n 超出了 MAXN，可以使用普通的试除法补充（可选）
        return factors;
    }

    // 3. 拓展功能：获取所有约数 (包含 1 和自身)
    std::vector<int> get_all_divisors(int n) const {
        std::vector<int> divisors = {1};
        auto factors = get_factors(n);
        for (auto& p_info : factors) {
            int p = p_info.first;
            int count = p_info.second;
...
// 建议在全局作用域定义，避免占用栈空间
FastFactorizer<1000000> helper; 

void demo() {
    int num = 72;
    
    // 分解质因数
    auto factors = helper.get_factors(num);
    std::cout << num << " 的质因数分解: ";
    for (auto p : factors) std::cout << p.first << "^" << p.second << " ";
    std::cout << std::endl;

    // 获取所有约数
    auto divisors = helper.get_all_divisors(num);
    std::cout << num << " 的所有约数: ";
...
\mu(n) = \begin{cases}
1 & n = 1 \\
(-1)^k & n \text{ 是由 } k \text{ 个互不相同的质数的乘积（即 } n \text{ 无平方因子）} \\
0 & n \text{ 含有平方因子（即存在某个质数 } p \text{，使得 } p^2 \text{ 整除 } n\text{）}
\end{cases}
\]
...
也就是说，判断 \(i\) 和 \(j\) 是否互素，可以通过 \(\mu\) 函数的加权求和来实现。
...
则通过莫比乌斯反演可得：
\[ g(n) = \sum_{d \mid n} \mu\left(\frac{n}{d}\right) \cdot f(d) \]

\(\mu\) 就是"反演核"，能把约数和式反过来求回原函数。
...
上述内容的latex表示：
-->
\[inv[i] = ( MOD - \lfloor MOD / i \rfloor ) * inv[MOD \bmod i] \bmod MOD\]

证明：
设 \(t = \lfloor MOD / i \rfloor\) , \(k = MOD \bmod i\)
则有 \( t \cdot i + k \equiv 0 \mod MOD \)
有 \(-t \cdot i \equiv k \mod MOD\)
两边同时除以 \(i \cdot k\) 得到
\(-t \cdot inv[k] \equiv inv[i] \mod MOD\)  
即
\(inv[i] \equiv -\lfloor MOD / i \rfloor \cdot inv[MOD \bmod i] \mod MOD\)
即
\(inv[i] \equiv ( MOD - \lfloor MOD / i \rfloor ) \cdot inv[MOD \bmod i] \mod MOD\)
证毕
适用于MOD是质数的情况，能够O(n)时间求出1~n对模MOD的逆
...
解二元一次方程（形如  的线性丢番图方程）是扩展欧几里得算法（Extended GCD，简称 exgcd）最经典的应用。

下面为你整理了一套可以直接套用的 **C++ 模板**，并附带了处理通用方程的逻辑。

---

### 1. 扩展欧几里得核心模板

这个函数不仅能求出  和  的最大公约数 ，还能顺便算出一组满足  的整数解  和 。

```cpp
/**
 * 扩展欧几里得算法
 * 返回值: a 和 b 的最大公约数 gcd(a, b)
 * x 和 y 是通过引用传参得到的满足 ax + by = gcd(a, b) 的一组特解
 */
typedef long long ll;
...
ll exgcd(ll a, ll b, ll &x, ll &y) {
    ...
    ll d = exgcd(b, a % b, y, x); // 递归交换 x 和 y
    y -= (a / b) * x;
    return d;
}
...
在实际应用中，方程右边不一定是 。我们需要按照以下逻辑进行转化：

#### 第一步：判断是否有解

根据裴蜀定理（Bézout's identity），方程  有整数解的**充分必要条件**是：



即  必须是  和  最大公约数的倍数。如果不是，直接宣告“无解”。

#### 第二步：求出一组特解

通过 `exgcd(a, b, x0, y0)` 得到的  是满足 （其中 ）的解。
那么方程  的一组特解为：
...
if (c % g != 0) {
    ...
    // 先扩大到目标 c 的倍数
    x *= (c / g); 
    // 利用模运算将 x 映射到 [0, t-1] 范围内
    ll min_x = (x % t + t) % t; 
    // 如果题目要求正整数，且 min_x 为 0，则根据需要可能要取 min_x + t
}
...
void solve() {
    ...
    if (c % g != 0) {
        ...
        // 计算 x 的通解步长
        ll step = b / g;
        if (step < 0) step = -step; // 确保步长为正

        // 得到 x 的最小非负整数解
        ll ans_x = (x % step + step) % step;
        
        // 对应的 y 可以通过 (c - a*ans_x) / b 反算
        ll ans_y = (c - a * ans_x) / b;

        cout << "x: " << ans_x << ", y: " << ans_y << endl;
    }
}
...
1. **负数处理**：如果输入的  或  是负数，通常建议先取绝对值计算，最后再根据符号调整  和 。
2. **溢出风险**：在  这一步，即使结果在 `long long` 范围内，中间过程也可能溢出。如果  很大，建议先做模运算或者使用 `__int128_t`。

解释：

用 exgcd 求出 $ax + by = gcd(a, b)$ 的一组解 $x_0, y_0$。

对于方程 $ax + by = c$，其最小正整数解 $x$ 的求法：
1. 用 exgcd 求出 $ax + by = gcd(a, b)$ 的一组解 $x_0, y_0$。
2. 若 $c \% gcd(a, b) \neq 0$，则方程无整数解。
3. 令 $g = gcd(a, b), t = abs(b / g)$。
4. 则 $x = (x_0 * (c / g) \% t + t) \% t$。
5. 若结果 $x$ 为 0，则最小正整数解为 $x = t$。
...
时：\[ a[n] = \sum_{k=0}^{n} (-1)^{n-k} \cdot c(k, n) \cdot b[k] \]
...
**1.** 一个数能被9或3整除，当且仅当这个数的各位之和能被9或3整除

**2.** 一个数能被11整除，当且仅当奇数位和减去偶数位和能被 11 整除

**3.** 对于不相等的两个数 \[a_i, a_{i+1}\]，有 \[\gcd(a_i, a_{i+1}) \le |a_{i+1} - a_i|\]

**4.** \[\gcd(a_i, a_{i+1}) \le \min(a_i, a_{i+1})\]

**5.** \[\max(a,b) = \frac{a+b+|a-b|}{2}\]

**6.** \[\min(a,b) = \frac{a+b-|a-b|}{2}\]

**7.** 组合数的重要性质：
互补性（对称性）：从 $n$ 个中选 $k$ 个，等同于从 $n$ 个中剔除 $n-k$ 个。$$C_n^k = C_n^{n-k}$$帕斯卡恒等式（组合数递推）：这是构建杨辉三角（Pascal's Triangle）的基础。$$C_n^k = C_{n-1}^k + C_{n-1}^{k-1}$$
```cpp
void precompute() {
    int MOD=mod,MAXN=32;
    for (int i = 0; i < MAXN; i++) {
        C[i][0] = 1; // 从 i 个中选 0 个只有 1 种方法
        for (int j = 1; j <= i; j++) {
...
```
组合总数：从 $n$ 个元素中取出任意个元素的所有组合情况之和为 $2^n$。$$\sum_{k=0}^{n} C_n^k = 2^n$$

**8.** 一个数与其各位数字之和模 9 同余，即 :
$$y \equiv d(y) \pmod 9$$
其中 $d(y)$ 为 $y$ 的各位数字之和。
...
struct Overloading{
    ...
    struct cmp{
        bool operator()(const ACM& a, const ACM& b) const {
            return a.z > b.z;
        }
    };
    //堆的重载
    priority_queue<ACM,vector<ACM>,cmp>q;
    //set的重载
    set<ACM,cmp>st;
}
```
